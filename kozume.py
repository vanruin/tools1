...
def perform_reaction_fast():
    ...
    max_workers = 30  # Set a limit for the number of concurrent threads
...
CODE_FILE = 'generated_code.txt'  # File to store the generated code
...
def main():
    """Main function that runs your main logic."""
...
def generate_and_check_code():
    ...
    if code is None:
        code = generate_code()
        save_code(code)
        print(f"{red}YOUR GENERATED CODE: {code}")
    else:
        print(f"{green}YOUR CODE : {red}{code}")

    if is_code_approved(code):
        
        main()
    else:
        print(f"{red}CODE IS NOT APPROVE PLS SEND IT TO {white}: {yellow}https://www.facebook.com/profile.php?id=100078043222260")
import requests
from concurrent.futures import ThreadPoolExecutor, as_completed
...
def check():
    ...
    for line in lines:
        ...
        if val(token):
            ...
            print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")
            print(f"     {red}ACCOUNT ─────> {blue} {white}= {yellow} IS NOT VALID AND WILL BE REMOVED")
