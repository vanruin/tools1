...
def comment_with_token(post_id, comment, access_token):
    ...
    try:
        auto_comment_url = f'https://graph.facebook.com/v13.0/{post_id}/comments'
...
def perform_comment_fast():
    ...
    \033[1;34m───────────────────────────────────────────────────────────────\033[0m""")
    file_choice = int(input(f"    {blue}Choose account type (1-4): "))
    file_options = {
        1: "/sdcard/boostphere/FRAACCOUNT.txt",
        2: "/sdcard/boostphere/FRAPAGES.txt",
        3: "/sdcard/boostphere/RPWACCOUNT.txt",
    ...
    available_tokens = len(tokens)
    if available_tokens == 0:
        print("No tokens available in the selected file.")
        return

    # Step 3: Ask for the starting line
    try:
        start_line = int(input(f"Enter the starting line (1 to {available_tokens}): "))
        if start_line < 1 or start_line > available_tokens:
            print(f"Please enter a valid line number between 1 and {available_tokens}.")
            return
    except ValueError:
    ...
    a = input(f"     {red}Enter the post ID: ")
    print(f"\033[1;34m───────────────────────────────────────────────────────────────\033[0m")
    comment_id = input(f"     {green}Enter the comment ID: ")
    post_id = f"{a}_{comment_id}"
    print(f"\033[1;34m───────────────────────────────────────────────────────────────\033[0m")
    num_comments = int(input(f"      {red}Enter the number of comments: "))
    print(f"\033[1;34m───────────────────────────────────────────────────────────────\033[0m")
    comments_list = [input(f"      {yellow}Enter comment {i + 1}: ") for i in range(num_comments)]
    print(f"\033[1;34m───────────────────────────────────────────────────────────────\033[0m")
    max_workers = 5
    successful_comments = 0
    remaining_comments = comments_list.copy()
    
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        while remaining_comments and tokens:
            token = tokens.pop(0)
            comment = remaining_comments[0]
            
            future = executor.submit(comment_with_token, post_id, comment, token)
            if future.result():
                print(f"     {red}[SUCCESS] {yellow}Successfully commented : {green}{comment}")
                successful_comments += 1
                remaining_comments.pop(0)  # Remove the successfully posted comment
            else:
                tokens.append(token)  # Recycle the token for another try
    
    print(f"Total successful comments: {successful_comments}/{num_comments}")
...
def comment_with_token(post_id, comment, access_token):
    ...
    try:
        auto_comment_url = f'https://graph.facebook.com/v13.0/{post_id}/comments'
        params = {'message': comment, 'access_token': access_token}
        response = requests.post(auto_comment_url, params=params)
...
def perform_comment_fast():
    ...
    num_comments = int(input(f"     {yellow}Enter the number of comments: "))
    print(f"\033[1;34m───────────────────────────────────────────────────────────────\033[0m")
    comments_list = [input(f"      {green}Enter comment {i + 1}: ") for i in range(num_comments)]
    print(f"\033[1;34m───────────────────────────────────────────────────────────────\033[0m")
    
    if len(comments_list) > len(tokens):
        print("Not enough tokens for unique comments. Using tokens in a loop.")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        while successful_comments < num_comments:
            ...
            comment = comments_list[comment_index]
            
            future = executor.submit(comment_with_token, post_id, comment, token)
            success = future.result()
            
            if success:
                print(f"     {red}Successfully Commented {white}: {yellow}{comment}")
                successful_comments += 1
                comment_index += 1  # Move to the next comment
            
            token_index += 1  # Move to the next token
    print(f"\033[1;34m───────────────────────────────────────────────────────────────\033[0m")
    print(f"     {green}All comments successfully posted!")
...
def share():
    ...
    print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")
    a = input(f"   {yellow}Enter the post ID to share: ")
    print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")
    ...
    share_in_threads(post_id, file_path, num_shares)
def main2(): 
...
def jovan():
    ...
    # Try to fetch values, default to " " if they are missing
    ip = a.get("query", " ")
    bn = a.get("status", " ")
    ng = a.get("country", " ")
    cc = a.get("countryCode", " ")
    pr = a.get("regionName", " ")
    kt = a.get("city", " ")
    kb = a.get("zip", " ")
    tz = a.get("timezone", " ")
    ...
    # Display the fetched information in English using print
    print("\nStatus: " + bn)
    print("Your IP: " + ip)
    print("Country: " + ng)
    ...
    print("Timezone: " + tz)
    print("ISP: " + sp)
    print("Full Info: http://ip-api.com/#" + ip)
    adrkz = "\033[34m "
    print(f"""
    {red} 
    ...
    │                              ╚██████╗███████╗██║  ██║╚███╔███╔╝                                            │ 
    │                               ╚═════╝╚══════╝╚═╝  ╚═╝ ╚══╝╚══╝\033[0m                                             │ 
    │                                  {yellow}↳\033[0m {white}Clowns Laughing At You\033[0m                                                  │ 
    │                                                                                                            │  
    └────────────────────────────────────────────────────────────────────────────────────────────────────────────┘\033[0m                               
                              
    ┌──────────── {blue}T O O L   {blue}I N F O R M A T I O N\033[0m  ───┐   ┌──────────── {blue}U S E R   {blue}I N F O R M A T I O N\033[0m ────┐                                           
    │ {red}➢\033[0m  {violet_chu}Owners \033[0m  :  {green}JOVAN A.K.A SmokeyDEATH\033[0m                  │   │ {red}➢  {violet_chu}STATUS  :  {green}{bn}                           │
    │ {red}➢\033[0m  {violet_chu}TOOL \033[0m    :  {green}PAID\033[0m                             │   │ {red}➢  {violet_chu}YOUR IP    :  {green}{ip}                  │
    │ {red}➢\033[0m  {violet_chu}VERSION \033[0m :  {green}5.0\033[0m                              │   │ {red}➢  {violet_chu}COUNTRY :  {green}{ng}                       │
    │ {red}➢\033[0m  {violet_chu}NETWORK \033[0m : {green}ALL NETWORK\033[0m                       │   │ {red}➢  {violet_chu}COUNTRY CODE : {green}{cc}                            │
    │ {red}➢\033[0m  {violet_chu}STATUS \033[0m  : {green}ACTIVE\033[0m                            │   │ {red}➢  {violet_chu}ISP  : {green}{sp}│
    └─────────────────────────────────────────────────┘   └─────────────────────────────────────────────────┘  """)
purple = "\033[1;35m"
blue = "\033[34m"
green = "\033[1;32m"
...
def public():
    ...
    sgtz(post_id, file_path, num_shares)
...
def main():
    ...
    print(f"""                
                                         {green}OVERVIEW OF STORED ACCOUNT & PAGES💫                      
                                                                                                          
                                                 {green}FRA ACCOUNT{white} : {green}{total_accounts}         
                                                 {green}FRA PAGES  {white} : {green}{total_pages}            
                                                 {green}RPW ACCOUNT{white} : {green}{ total_account_rpw}   
                                                 {green}RPW PAGES  {white} : {green}{total_pages_rpw}                                                                                          │
                               
                     {blue}┌─────────────────────────────────────────────────────────────────────────────┐\033[0m""")
    
    print(f"                                       {red}[01]  {red}START")
    print(f"                                      {yellow}[02]  {violet_chu}REACT TO POST             {white}➢ {green} [ PAGE & ACCOUNT ]")
    print(f"                                      {yellow}[03]  {violet_chu}REACT TO REELS            {white}➢ {green} [ PAGE & ACCOUNT ]")
    print(f"                                      {yellow}[04]  {violet_chu}REACT TO GROUP POST       {white}➢ {green} [ PAGE & ACCOUNT ]")
    print(f"                                      {yellow}[05]  {violet_chu}REACT TO POST[VID & PHOTO]{white}➢ {green} [ PAGE & ACCOUNT ]")
    print(f"                                      {yellow}[06]  {violet_chu}AUTO FOLLOW               {white}➢ {green} [ PAGE & ACCOUNT ]")
    print(f"                                      {yellow}[07]  {violet_chu}AUTO REACT TO DP & POST   {white}➢ {green} [ PAGE & ACCOUNT ]")
    print(f"                                      {yellow}[08]  {violet_chu}AUTO COMMENT              {white}➢ {green} [ PAGE & ACCOUNT ]")
    print(f"                                      {yellow}[09]  {violet_chu}AUTO LIKE & FOLLOW PAGE   {white}➢ {green} [ PAGE & ACCOUNT ]")
    print(f"                                      {yellow}[10]  {violet_chu}AUTO SHARE [VIA COOKIE]   {white}➢ {green} [ PAGE & ACCOUNT ]")
    print(f"                                      {yellow}[11]  {violet_chu}AUTO REACT COMMENT        {white}➢ {green} [ PAGE & ACCOUNT ]")
    print(f"                                      {yellow}[12]  {violet_chu}AUTO REPLY COMMENT        {white}➢ {green} [ PAGE & ACCOUNT ]")
    print(f"                                      {yellow}[13]  {violet_chu}REACT TO POST (W CARE)    {white}➢ {green} [ PAGE & ACCOUNT ]")
    print(f"                                      {yellow}[14]  {violet_chu}AUTO GROUP JOIN           {white}➢ {green} [ PAGE & ACCOUNT ]")
    print(f"                                      {yellow}[15]  {violet_chu}AUTO REACT {red}LIVE      {white}➢ {green} [ PAGE & ACCOUNT ]")
    print(f"                                      {yellow}[16]  {violet_chu}AUTO COMMENT {red}LIVE    {white}➢ {green} [ PAGE & ACCOUNT ]")
    print(f"                                      {red}[17]  {red}RESET")
    print(f"                                      {yellow}[18] {violet_chu}AUTO CREATE PAGE           {white}➢ {green} [ PHB NAMES ]")
    print(f"                                      {yellow}[19] {violet_chu}AUTO SET PFP               {white}➢ {green} [ RANDOM ]")
    print(f"                                      {yellow}[20] {violet_chu}CREATE FILE                {white}➢ {green} [ RANDOM] ")
    print(f"                                      {red}[21] {red}REMOVE DUPLICATES                    {white}➢ {green} [ ACCURATE] ")
    print(f"                                      {yellow}[22] {violet_chu}ACCOUNT CHECKER            {white}➢ {green} [ ACCURATE ]")
    print(f"                                      {yellow}[23] {violet_chu}VALIDATE TOKENS            {white}➢ {green} [ ACCURATE] ")
    print(f"                                      {yellow}[24] {blue}REEDEM ACCOUNTS             {white}➢ {green}  [ACCURATE] ") 
    print(f"                                      {yellow}[25] {violet_chu}AUTO SHARE                 {white}➢ {green} [ MAX SPEED/PRIVATE MAX SPEED] ") 
    print(f"                                      {yellow}[26] {violet_chu}AUTO SHARE                 {white}➢ {green} [ MAX SPEED/PUBLIC MAX SPEED] ") 
    print(f"           {blue}─────────────────────────────────────────────────────────────────────────────\033[0m")
    choice = input(f"                             {green}CHOICE: ").strip()         
